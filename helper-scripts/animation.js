...
function getChatWindowInfo(windowElement) {
    ...
    window["chatbox"] = document.getElementById("chatbox-" + windowId)
...
function addOrRemoveChat(timing, currentTime) {
    const { txt, timeOut, startTime } = getTextTiming(timing, currentTime)

    // Show/hide this/other windows
    setTimeoutWrapper(
        () => { showWindow(timing.windowId, timing.show, /* exclusive=*/false) },
        () => { }, startTime);
}
...
function switchChat(timing, currentTime) {
    ...
    // Show/hide this/other windows
    setTimeoutWrapper(
        () => { showWindow(timing.windowId, /* show= */true, /* exclusive=*/true) },
        () => { /* showWindow(timing.windowId, false, true) */ }, startTime);
}
...
function renameChat(timing, currentTime) {
    ...
    setTimeoutWrapper(() => {
        chatTitle.get(windowId).innerText = txt;
    }, () => { }, startTime)
}
...
function makeTimings() {
    ...
    for (let i = 0; i < chatMessages.length; i++) {
        ...
        const defaultPrompt = ELEMENTS.chatwindows.get(windowId)["default-prompt"].textContent
...
function showWindow(windowId, show, exclusive) {
    if (show) {
